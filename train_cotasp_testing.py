...
flags.DEFINE_integer('calculate_layer_sensitivity_interval', int(1e4), 'calculate the layer sensitivity every x steps')
    ...
    for task_idx, dict_task in enumerate(seq_tasks):
        ...
        for idx in range(FLAGS.max_step):
            ...
            if idx % FLAGS.calculate_layer_sensitivity_interval == 0 and idx > 0 and idx < FLAGS.stop_reset_after_steps:
                ...
                frozen_neuron_mask = frozen_neuron_mask(agent.cumul_masks, info)
                ...
                each_layer_reset_indices = top_k_neuron(layer_neuron_difference, k=10)
...
# def calculate_layer_neuron(observations, action, critic):
#     q, intermediate = critic(observations, action)
#     # noise = jax.random.normal(jax.random.PRNGKey(0), shape=(observations.shape[0], 12))
#     noise_observations = observations.mean(axis=0) * 0.01
#     noise_actions = action.mean(axis=0) * 0.01
#     # Add the noise to the observations
#     q_1, perturbed_observations = observations.at[:, :12].add(noise_observations)
#     perturbed_actions = action.at[:, :4].add(noise_actions)
#     perturbed_intermediate = critic(perturbed_observations, perturbed_actions)
#     delta_q = tree_map(lambda x, y: jnp.abs(x - y), intermediate['intermediates'], perturbed_intermediate['intermediates'])
#     return delta_q
def top_k_neuron(layer_neuron_difference, k=10):
    """
